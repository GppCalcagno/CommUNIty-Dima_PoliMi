...
import 'package:dima_project/Atest_lib/widgets/group_chat/chat_bubble_widget.dart';
...
class ChatWidgetState extends State<ChatWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.group == null) {
      return Column(
        ...
        children: [
          Image.asset("assets/chat.png", height: 550, width: 550),
        ]
      );
    } else {
                        ...
                        child: Icon(Icons.info_outline_rounded, color: Colors.white70),
                          onTap: () {
                            showDialog(
                              ...
                              builder: (context) {
                                return AlertDialog(
                                  icon: Icon(key: Key('groupInfoIcon'), Icons.groups_rounded, size: 60, color: Colors.deepPurple[400]),
        ...
        children: [
          Container(
            ...
            child: IconButton(
              ...
              onPressed: () async {
                /*_gif = await GiphyPicker.pickGif(
                  context: context, 
                  apiKey: '');
                if (_gif != null) {
                  _sendMessage(true, _gif!.images.original!.url.toString(), Timestamp.now(), FirebaseFirestore.instance.doc("users/${widget.currentUser.uid}"));
          ...
          GestureDetector(
            onTap: () {
              //_sendMessage(false, _messageTextController.text, Timestamp.now(), FirebaseFirestore.instance.doc("users/${widget.currentUser.uid}"));
              //_sendPushNotification();
            },
            child: Container(
              key: Key('sendButton'),
  ...
  /*_sendMessage(bool isGif, String message, Timestamp timestamp, DocumentReference sender) async {
    if (!isGif && _messageTextController.text.isNotEmpty || isGif) {
      MessageModel msg = _buildMessage(isGif, message, timestamp, sender);

      await dbService.sendMessage(widget.group!.groupId, msg);      
